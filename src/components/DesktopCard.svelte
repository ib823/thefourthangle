...
<div
  ...
  style="
    ...
    transform:{visible ? (hovered ? 'translateY(-3px) scale(1.006)' : 'translateY(0) scale(1)') : 'translateY(16px) scale(0.96)'};
    opacity:{visible ? 1 : 0};
    transition:transform 0.35s cubic-bezier(.175,.885,.32,1.275), opacity 0.35s ease, box-shadow 0.2s ease;
  "
>
  <div style="display:flex;align-items:center;gap:6px;">
    ...
    {#if isStarted}
      <span style="font-size:10px;font-weight:700;color:#B85C00;background:#FFF4E6;padding:2px 7px;border-radius:6px;">{readState.progress}/6</span>
    {:else if isCompleted}
      <div style="width:18px;height:18px;border-radius:50%;background:#EBFBEE;display:flex;align-items:center;justify-content:center;">
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#2B8A3E" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
      </div>
    {/if}
  </div>
  ...
  <div style="display:flex;align-items:center;gap:8px;margin-top:12px;">
    ...
    {:else}
      <div style="display:flex;align-items:center;gap:3px;">
        {#each dotColors as c}
          <div style="width:4px;height:4px;border-radius:50%;background:{c};opacity:{hovered ? 1 : 0.4};transition:opacity 0.2s ease;"></div>
        {/each}
        <span style="font-size:10px;color:#6C757D;margin-left:4px;opacity:{hovered ? 1 : 0};transition:opacity 0.2s ease;">6 ideas</span>
      </div>
    {/if}
  </div>
</div>
